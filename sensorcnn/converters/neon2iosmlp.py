...
def extract_weights(file_name):
    ...
    layers = params["model"]["config"]["layers"]
    ...
    for layer_idx in range(0, len(layers), 3):  # 3 because one layer contains [linear, bias, activation]
        # Make sure our model has biases activated, otherwise add zeros here
        w = layers[layer_idx]['params']['W']
        b = layers[layer_idx + 1]['params']['W']
